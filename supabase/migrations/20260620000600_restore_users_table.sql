...
-- Note: team_members table already exists (with 's'), no need to rename
...
-- Ensure team_members table exists with correct structure (it should already exist)
-- If it doesn't exist, create it
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'team_members' AND table_schema = 'public') THEN
